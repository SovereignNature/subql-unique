...
import {
  collectionExist,
  getCollectionOrElseCreate,
  getTokenOrElseCreate,
} from "./utils/getter";
...
export async function handleForceCreateCollection(
  event: SubstrateEvent
): Promise<void> {
  const collection = processCollection(event.extrinsic);
...
export async function handleAssetStatusChange(
  ...
  const [id, newOwner, newIssuer, newAdmin, newFreezer, , frozen] = getArgs(
    extrinsic.args,
    [0]
  );
  const caller = getSigner(event);
  ...
  try {
    ...
    logger.warn(
      `!! BAD ROOT in block ${JSON.stringify(other, null, 2)}:  ${e.message}`
    );
  }
}
...
export async function handleTokenTransfer(
  ...
  if (
    isEmpty(
      ...
      `Token Transfer ${finalId} [collectionId]`
    )
  ) {
    return;
  }
...
export async function handleTokenFreeze(event: SubstrateEvent): Promise<void> {
  ...
  const caller = getSigner(event);
  const finalId = createTokenId(collectionId, id);
  const final = await getTokenOrElseCreate(finalId, caller);
  ...
  if (
    isEmpty(
      collectionExist(collectionId),
      `Token Freeze ${finalId} [collectionId]`
    )
  ) {
    return;
  }
...
export async function handleTokenMetadata(
  ...
  if (
    isEmpty(
      collectionExist(collectionId),
      `Token Metadata ${finalId} [collectionId]`
    )
  ) {
    return;
  }
...
export async function handleTokenMetadataClear(
  ...
  if (
    isEmpty(
      collectionExist(collectionId),
      `Token Metadata Clear ${finalId} [collectionId]`
    )
  ) {
    return;
  }
...
export async function handleTokenApproval(
  ...
  if (
    isEmpty(
      collectionExist(collectionId),
      `Token Approval ${finalId} [collectionId]`
    )
  ) {
    return;
  }
